...
void MB_ReadHoldingReg_BATTERY()
{
	uint16_t TxCount = 0;
  uint16_t crc = 0;
	Tx_Buf[TxCount++] = 0xDD;		    /* 从站地址 */
	Tx_Buf[TxCount++] = 0xA5;		      /* 功能码 */	
	Tx_Buf[TxCount++] = 0X03;	  /* 寄存器地址 高字节 */
	Tx_Buf[TxCount++] = 0X00;		      /* 寄存器地址 低字节 */
	Tx_Buf[TxCount++] = 0XFF;	  /* 寄存器(16bits)个数 高字节
  
	crc = MB_CRC16((uint8_t*)&Tx_Buf,TxCount);
  Tx_Buf[TxCount++] = crc;	          /* crc 低字节 */
  Tx_Buf[TxCount++] = 0XFD;		      /* 寄存器地址 低字节 */
	Tx_Buf[TxCount++] = 0X77;		      /* 寄存器地址 低字节 */;	 


  UART_Tx((uint8_t *)&Tx_Buf,TxCount);
}
void MB_ReadHoldingReg_QID()
{
	uint16_t TxCount = 0;
  uint16_t crc = 0;
	Tx_Buf[TxCount++] = 0x21;		    /* 从站地址 */
	Tx_Buf[TxCount++] = 0x0F;		      /* 功能码 */	
	Tx_Buf[TxCount++] = 0X00;	  /* 寄存器地址 高字节 */
	Tx_Buf[TxCount++] = 0X00;		      /* 寄存器地址 低字节 */
	Tx_Buf[TxCount++] = 0X00;	  // 寄存器(16bits)个数 高字节
  	Tx_Buf[TxCount++] = 0X0C;	  // 寄存器(16bits)个数 高字节
		Tx_Buf[TxCount++] = 0X02;	  // 寄存器(16bits)个数 高字节
		Tx_Buf[TxCount++] = 0X0C;	  // 寄存器(16bits)个数 高字节
		Tx_Buf[TxCount++] = 0X00;	  // 寄存器(16bits)个数 高字节
		crc = MB_CRC16((uint8_t*)&Tx_Buf,TxCount);
  Tx_Buf[TxCount++] = crc;	          /* crc 低字节 */
	Tx_Buf[TxCount++] = crc>>8;		      /* crc 高字节 */          /* crc 低字节 */
  //Tx_Buf[TxCount++] = 0X79;		      /* 寄存器地址 低字节 */
	//Tx_Buf[TxCount++] = 0X71;		      /* 寄存器地址 低字节 */;	 


  UART_Tx((uint8_t *)&Tx_Buf,TxCount);
}
void MB_ReadHoldingReg_DATE(uint8_t _addr, uint16_t _date1, uint16_t _date2, uint16_t _date3 ,uint16_t _date4 , uint8_t *_databuf)
{
	uint16_t TxCount = 0;
  uint16_t crc = 0;
	Tx_Buf[TxCount++] = _addr;		    /* 从站地址 */
	Tx_Buf[TxCount++] = 0x03;		      /* 功能码 */	
  Tx_Buf[TxCount++] = _date1 >> 8;	  /* 寄存器地址 高字节 */
	Tx_Buf[TxCount++] = _date1;		      /* 寄存器地址 低字节 */
  Tx_Buf[TxCount++] = _date2 >> 8;	  /* 寄存器(16bits)个数 高字节 */
	Tx_Buf[TxCount++] = _date2;		      /* 低字节 */
  Tx_Buf[TxCount++] = _date3>> 8;		      /* 寄存器地址 低字节 */
  Tx_Buf[TxCount++] = _date3;		      /* 寄存器地址 低字节 */
  Tx_Buf[TxCount++] = _date4;		      /* 寄存器地址 低字节 */
